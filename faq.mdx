...
Yes. The SDK includes built-in analytics tracking and exposes `Tallysight.trackCustomEvent(name, props?, domain?)` for custom event tracking. The SDK tracks page views automatically when Tallysight widgets are present on a page.
</Accordion>
