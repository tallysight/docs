...
Auto-detection uses a MutationObserver to watch the DOM for newly added widget elements. When something adds a widget element dynamically — for example, a SPA router, a content management system rendering blocks on the fly, or an AJAX request — the SDK automatically detects it and loads the corresponding widget bundle. No extra code is required on your part.
