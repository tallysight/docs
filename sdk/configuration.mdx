...
The SDK reads configuration from `data-` attributes on the `<script>` tag at load time. You can also update configuration at runtime by calling `Tallysight.configure()`. The SDK applies script tag attributes first, then lets programmatic calls override them at any point after initialization.
