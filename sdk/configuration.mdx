...
The SDK reads configuration from `data-` attributes on the `<script>` tag at load time, and you can also update it at runtime using `Tallysight.configure()`. The SDK applies script tag attributes first; programmatic calls override them at any point after the SDK initializes.
