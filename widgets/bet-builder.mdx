...
<Note>
The SDK automatically creates and inserts the bet builder widget element into the DOM when it initializes with `data-tallysight-bet-builder-enabled="true"` or when you call `Tallysight.enableBetBuilder()`. Calling `Tallysight.disableBetBuilder()` removes the widget element from the DOM and clears the bet store state. You do not need to add a `data-tallysight-widget-type="bet-builder"` element manually.
</Note>
