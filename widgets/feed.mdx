...
The Feed widget displays a scrollable, infinitely paginating list of betslips published by analysts in your workspace. Visitors can filter by sport and event. You can also configure multiple tabs to let them switch between all picks, verified analyst picks, and community picks. The feed loads three betslips at a time and fetches more as the visitor scrolls.
...
If you only supply a single tab value, the widget does not show a tab bar — it simply requests that feed type from the API. If you omit `config-tabs` entirely, the widget falls back to its default behavior and sends no tab parameter to the API. This keeps older embeds backward compatible.
...
| `config-community-scope` | `workspace` | `workspace` limits community picks to your embed workspace; `platform` shows community picks from all of Tallysight |
